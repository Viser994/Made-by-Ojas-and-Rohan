...
import { InsertUser, users, responses, InsertResponse } from "../drizzle/schema";
...
export async function createResponse(data: InsertResponse): Promise<void> {
  const db = await getDb();
  if (!db) {
...
export async function getAllResponses() {
  ...
  try {
    ...
    console.error("[Database] Failed to get responses:", error);
    return [];
  }
}
